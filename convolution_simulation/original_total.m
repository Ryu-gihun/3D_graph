...
enc_result = zeros(1, 2*(in_num+k-1));
I = in_num*2+k-1;
state = zeros(1,I);
...
for n = 1:k
    if n==1
        ...
        max_a = 2^(n-1)-1;
        while aa <= max_a
            state(1,:) = map(2^(n-1)+aa,:);
            state = circshift(state,1);
            state(1,1) = 0;
            for r = 1:k
                map(2^n+2*aa,r) =  state(1, r);
            end
            state(1,1) = 1;
            for r = 1:k
                map(2^n+2*aa+1,r) =  state(1, r);
            end
            aa = aa + 1;
        end
        aa = 0;
...
while(error_stack == 0)
    %incoder
    start_i = randi([0 1], 1, in_num);

    for a=1:(in_num+k-1)
        x=0;
        y=0;
        if a<= in_num
            state_e(1,1) = start_i(1,a);
        else
            state_e(1,1) = 0;
        end
        for b=1:k
    ...
    noise = normrnd(0, sigma, [1, length(enc_result)]);
    o = output + 2*noise;
    N = size(output,2);
    e = 0;
